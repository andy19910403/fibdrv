...
{
    int len = (bnfib->size * 8 * sizeof(int)) / 3 + 2;
    ...
    // printf("bnfib->size=%d\r\n",bnfib->size);
    for (int i = bnfib->size - 1; i >= 0; i--) {
        for (unsigned int d = 1U << 31; d; d >>= 1) {
            int carry = !!(d & bnfib->number[i]);

            // printf("carry=%d D=%x
            // bnfib->data[i]=%x\r\n",carry,d,bnfib->data[i]);
...
char *bn
    for (int i = src.size - 1; i >= 0; i--) {
        for (unsigned int d = 1U << 31; d; d >>= 1) {
            ...
            for (int j = len - 2; j >= 0; j--) {
                s[j] += s[j] - '0' + carry;  // double it
       kstyle: The function 'fib_fast_double_iterative' is never usedip leading
zero while (p[0] == '0' && p[1] != '\0') { p++;
    }
    if (src.sign)
        *(--p) = '-';
    memmove(s, p, strlen(p) + 1);
